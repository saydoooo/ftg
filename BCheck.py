...
# meta title: BCheck
# meta pic: https://img.icons8.com/fluency/48/000000/cat-eyes.png
# meta desc: Bulk chat phone number leakage check
...
class BCheckMod(loader.Module):
    ...
    async def bcheckcmd(self, message: Message) -> None:
        ...
        async for user in message.client.iter_participants(message.peer_id):
            dt = requests.get(
                f"http://api.murix.ru/eye?v=1.2&uid={str(user.id)}"
            ).json()
            ...
            if "NOT_FOUND" not in dt:
                check_result += (
                    ...
                    + str(user.id)
                    + '}">'
                    + (str(user.first_name) + " " + str(user.last_name)).replace(
                        " None", ""
                    )
                    + "</a>: <code>"
                    + dt
    ...
    async def bchecksilentcmd(self, message: Message) -> None:
        """Silent mode of bcheck"""
        await message.delete()
        msg = await message.client.send_message(
            "me", self.strings("check_started", message)
        )
        check_result = self.strings("search_header", message)
